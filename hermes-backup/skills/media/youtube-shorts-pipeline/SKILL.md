...
- **Correction (2026-06-11):** Previous comments incorrectly stated 100-200 words. The builder enforces **50-100 words** via `len(subtitle.split())`. Our MW4 narration required trimming from ~127 → 96 words over 3 attempts. The 50-100 range is correct.
...
- `--subtitle` must be 50-100 words. The builder enforces this at runtime in `_check_subtitle_words()` and aborts before download/TTS if outside the range.
...
- **Duration issue (June 15, 2026)**: 79-word narration at `+25%` TTS rate produces ~23s — **still under 30s minimum**. Options: increase narration to 95-100 words, or slow TTS rate in `shorts_builder.py` (`--rate +25%` → `+0%` or `-10%`).
- Subtitle word count clarification: The builder's `_check_subtitle_words()` enforces **50-100 words** (not 100-200 as some comments state). Count with `len(subtitle.split())` before submitting. Our MW4 narration required trimming from ~127 → 100 words over 3 attempts.
...
The narration text passed to `src/shorts_builder.py --subtitle` must be under 100 words.
Keep the script concise — under 100 words — while still preserving the required opening/closing structure.
